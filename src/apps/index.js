...
const getApps = async (req) => {
  ...
  try {
    const apps = (await App.find({ user: userId })).map((app) => {
...
const createApp = async (req) => {
  ...
  try {
    ...
    await app.save();
    const {
      secret_hash, user, updated_at, ...rest
    } = app.toObject();
...
const refreshSecret = async (req) => {
  ...
  try {
    const Rack = hat.rack(); // Create client_secret
    const secret = Rack();
    const app = await App.findOne({ _id: id, user: userId });
